    ...
    <>
      ...
      <section className="bg-white py-8 antialiased dark:bg-gray-800 md:py-16">
        <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
          ...
          {
            lockedproduct.length > 0  ? lockedproduct.map((details)=>{
            {
...
}): <div className="text-2xl font-semibold text-white flex justify-center items-center">  No Products to Display. &nbsp;<Link to={"/productgrid"} className="text-violet-600 font-bold text-3xl">Shop Now...</Link> </div>}
          ...
          <div className="flex flex-col gap-8 p-8 w-full">{
            lockedproduct.length > 0  ? lockedproduct.map((product)=>{
             <div className="border-[1px] rounded-xl p-4 flex gap-10">
              <img className="rounded-lg w-80 h-full" src={`http://localhost:3001/${product.ImageLink}`}></img>
